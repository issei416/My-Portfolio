...
export default function Earth(props) {
  // Import the GLTF model with animations // Replace with actual path to your Earth glTF model
  const group = useRef();
  const { scene, animations } = useGLTF("/earth.gltf");
  ...
  useEffect(() => {
    ...
    if (actions && actions[Object.keys(actions)[0]]) {
      actions[Object.keys(actions)[0]].play(); // Plays the first animation available in the model
    }
  }, [actions]);

  useFrame(() => {
    group.current.rotation.y -= 0.002; // Adjust the speed of rotation here
  });

  return (
    <group ref={group} {...props} dispose={null}>
      {/* Replace with the JSX structure of your Earth model */}
      <group ref={group} {...props} dispose={null}>
      ...
      />
      {/* Add more nodes if there are other parts of the model */}
    </group>
  );
}

useGLTF.preload("earth.gltf"); // Preload the model to ensure faster rendering
